use crate::values::*;
use std::{cell::RefCell, pin::Pin, sync::Arc, time::SystemTime, ops::DerefMut, mem::MaybeUninit, io::{Cursor, Write}};
use tracelogging::*;
...
impl<'a> CommonSchemaPartCBuilder<'a> {
    fn make_visitor(eb: &'a mut tracelogging_dynamic::EventBuilder) -> VisitorWrapper<CommonSchemaPartCBuilder<'a>> {
    ...
    fn add_field_value(&mut self, fv: &FieldAndValue) {
        ...
        match fv.value {
            ...
            ValueTypes::v_u128(u) => {
                ...
                self.eb.add_binary(field_name, u.to_le_bytes(), OutType::Default, 0);
            },
            ValueTypes::v_i128(i) => {
                // Or maybe add_binaryc?
                self.eb.add_binary(field_name, i.to_le_bytes(), OutType::Default, 0);
            },
            ValueTypes::v_f64(f) => {
            ...
            ValueTypes::v_str(ref s) => {
                self.eb
                    .add_str8(field_name, s.as_ref(), OutType::Utf8, 0);
            }
            ValueTypes::v_char(c) => {
        ...
        EBW.with(|eb| {
            ...
            eb.add_struct("PartC", partc_field_count, 0);
            {
                ...
                for (f, v) in fields.iter().zip(values.iter()) {
                    <CommonSchemaPartCBuilder<'_> as AddFieldAndValue<CommonSchemaPartCBuilder<'_>>>::add_field_value(&mut pfv, &FieldAndValue {
                        field_name: f,
                        value: v,
                    });
                }
